...
/* 
Android is a 12 year old italian paster who can't code for shit he always begs homeless for help
*/
...
VOID MenuAndDestroy(ImGuiWindow& window) {
	...
	if (ShowMenu) {
		...
		if (ImGui::Begin(xorstr("Linora Beta"), &ShowMenu, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse )) {
			...
			if (maintabs == 0) {
				...
				ImGui::Checkbox(xorstr("Silent Aimbot [RISK]"), &Settings::SilentAim);
				ImGui::SameLine();
				ImGui::TextColored(ImColor(255, 231, 94, 255), xorstr("[?]"));
				if (ImGui::IsItemHovered()) {
					ImGui::BeginTooltip();
					ImGui::Text(xorstr("You do not have to aim at the opponent for the bullet to him them"));
					ImGui::EndTooltip();
				}
				...
				if (ImGui::IsItemHovered()) {
					ImGui::BeginTooltip();
					ImGui::Text(xorstr("You can aim and jump at the same time"));
					ImGui::EndTooltip();
				}
				ImGui::Checkbox(xorstr("Fov 360"), &Settings::fov360);
				ImGui::SameLine();
				ImGui::TextColored(ImColor(255, 231, 94, 255), xorstr("[?]"));
				if (ImGui::IsItemHovered()) {
					ImGui::BeginTooltip();
					ImGui::Text(xorstr("The aimbot goes even on players behind you"));
					ImGui::EndTooltip();
				}
				ImGui::Checkbox(xorstr("Trigger Bot"), &Settings::trigger);
				if (ImGui::Button(xorstr("First Camera"))) {
				...
				if (ImGui::IsItemHovered()) {
					ImGui::BeginTooltip();
					ImGui::Text(xorstr("First Person Shooter"));
					ImGui::EndTooltip();
				}
				ImGui::Checkbox(xorstr("Rapid Fire"), &Settings::RapidFire);
				ImGui::SameLine();
				ImGui::TextColored(ImColor(255, 231, 94, 255), xorstr("[?]"));
				if (ImGui::IsItemHovered()) {
					ImGui::BeginTooltip();
					ImGui::Text(xorstr("Shoots your weapon faster"));
					ImGui::EndTooltip();
				}
				if (Settings::RapidFire) {
					ImGui::SliderFloat(xorstr("Rapid Fire Value"), &Settings::RapidFireValue, 0.00001f, 0.1f);
				}

				//ImGui::Checkbox(xorstr("Rainbow Mode"), &RainbowMode);
			}
			else if (maintabs == 3) {
...
bool InitializeHack()
{
	...
	//uintptr_t sceneAddr = MemoryHelper::PatternScanW(DiscordHelper::module, xorstr("56 57 53 48 83 EC 30 44 89 C6"));

	//MessageBoxA(NULL, "Before PresentOriginal", "", MB_OK);

	//present_original = (decltype(present_original))HookHelper::InsertHook((BYTE*)present_vTable, (BYTE*)present_hooked, 19);
	//MessageBoxA(NULL, "Before ResizeOriginal", "", MB_OK);
	//resize_original = (decltype(resize_original))HookHelper::InsertHook((BYTE*)resize_vTable, (BYTE*)resize_hooked, 19);
...
bool InitMain() {
	LI_FN(GlobalAddAtomA)(xorstr("000c285457fc971f862a79b786476c78812c8897063c6fa9c045f579a3b2d63f"));
	//if (GlobalFindAtomA(xorstr("b36PXQ3KhzKvfrAz1")) == 0 || GlobalFindAtomA(xorstr("h42mVe8AaHStrpVr3")) == 0 || GlobalFindAtomA(xorstr("aTc5j7Zj6ZjNKqGC5")) == 0 || GlobalFindAtomA(xorstr("dfshah51t5agrgh")) == 0 || GlobalFindAtomA(xorstr("kdfhjadhk39rjasggas")) == 0)
	if (LI_FN(GlobalFindAtomA)(xorstr("dfshah51t5agrgh")) == 0 || LI_FN(GlobalFindAtomA)(xorstr("kdfhjadhk39rjasggas")) == 0)
	{
		
		SpoofCall(MessageBoxA, (HWND)0, (LPCSTR)xorstr("[Error] Open a ticket with error: 0x230"), (LPCSTR)xorstr("Error"), (UINT)MB_ICONERROR);
		return false;
	}
	else {
		LI_FN(GlobalAddAtomA)(xorstr("eb3cad5b7bea92b5831965ed33d976b1f1c192d69a4e34c9ce6385ce87fa1d34"));
		InitializeHack();
		return true;
	}
}
